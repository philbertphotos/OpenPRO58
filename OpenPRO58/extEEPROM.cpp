...
// workaround, BUFFER_LENGTH is not defined in Wire2.h for SAMD controllers
#ifndef BUFFER_LENGTH
#define BUFFER_LENGTH 32
#endif


HardWire Wire2(2,0x50);
...
{
    Wire2.begin();
    Wire2.setClock(twiFreq);
    Wire2.beginTransmission(_eepromAddr);
    if (_nAddrBytes == 2) Wire2.write((byte)0);      //high addr byte
    Wire2.write((byte)0);                            //low addr byte
...
byte extEEPROM::write(unsigned long addr, byte *values, unsigned int nBytes)
{
    ...
    while (nBytes > 0) {
        ...
        Wire2.beginTransmission(ctrlByte);
        if (_nAddrBytes == 2) Wire2.write( (byte) (addr >> 8) );   //high addr byte
        Wire2.write( (byte) addr );                                //low addr byte
        Wire2.write(values, nWrite);
        txStatus = Wire2.endTransmission();
...
byte extEEPROM::read(unsigned long addr, byte *values, unsigned int nBytes)
{
    ...
    while (nBytes > 0) {
        ...
        if (_nAddrBytes == 2) Wire2.write( (byte) (addr >> 8) );   //high addr byte
        Wire2.write( (byte) addr );                                //low addr byte
        rxStatus = Wire2.endTransmission();
        if (rxStatus != 0) return rxStatus;        //read error

        Wire2.requestFrom(ctrlByte, nRead);
        for (byte i=0; i<nRead; i++) values[i] = Wire2.read();
